...
不带参数，返回一张 PNG 指令速查图，覆盖全部 25 个注册命令、正式别名、素材类型和关键参数。图中会特别标明动图裁剪的位置参数、`gif截取` 的删帧/保留范围语义、时间和边距写法；完整的参数边界和配置说明仍以本 README 为准。

速查图作为插件内置 PNG 资源随发布包提供，不依赖 AstrBot 容器安装中文字体。
