...
`astrbot_plugin_gif_toolbox` 是面向 AstrBot 的本地图片与动画处理插件，支持 GIF/APNG/WebP 变速、整体裁剪、按帧截取、动图信息查看、静态图转 GIF、基础图片变换、精灵图合成、网格裁剪、分解和视频转动画。
...
回复 GIF、APNG 或 WebP 动图后使用。插件会将完整画面帧和对应的帧时长一并倒序，输出 GIF；静态图片不支持。输入帧数超过 `max_frames` 时，仍会先按该上限均匀采样，再倒放采样后的动画。
...
这是针对 GIF/APNG/WebP 动图的**整体裁剪**命令。它在所有帧上使用同一个裁剪框，保留动图的帧顺序和帧时长，并输出 GIF；它不会拆帧，也不会像 `/裁剪` 一样生成网格小图。

| 模式 | 写法 | 行为 |
| --- | --- | --- |
| 固定比例 | `/gif裁剪 1:1`、`/gif裁剪 比例 16:9` | 从原画布中取满足比例的最大区域，不缩放内容。比例两个数均为 `1` 到 `10000`。 |
| 固定尺寸 | `/gif裁剪 尺寸 512x512` | 裁出精确像素尺寸，不缩放。宽高各为 `1` 到 `4096`，且不能大于处理后的动图画布。 |
| 去除边缘 | `/gif裁剪 边距 20`、`/gif裁剪 左边距 40 右边距 40` | 按像素从指定边缘裁掉范围；通用边距和单边边距的组合规则与精灵图合成相同。 |

比例或尺寸模式可在末尾追加裁剪位置：`左上`、`上`、`右上`、`左`、`居中`、`右`、`左下`、`下`、`右下`，省略时为 `居中`。边距模式不需要位置参数，且不能与比例、尺寸模式混用。
...
静态图片不支持该命令。动图会先遵守 `max_image_side` 和 `max_frames` 限制；若输出仍过大，插件会按 `max_output_size_mb` 自动压缩。
...
这是时间轴上的裁剪，和画面范围的 `/gif裁剪` 不同。它只对 GIF/APNG/WebP 动图生效，保留原帧的顺序和每帧时长，输出 GIF。

| 写法 | 含义 |
| --- | --- |
| `/gif截取 前10帧` | **删除**最前面的 10 帧，保留之后所有帧。 |
| `/gif截取 后10帧` | **删除**最后面的 10 帧，保留之前所有帧。 |
| `/gif截取 前10帧 后5帧` | 同时删除前 10 帧和后 5 帧。 |
| `/gif截取 10-60帧` | 只保留第 10 到第 60 帧，帧号从 1 开始且两端都包含。也可写 `第10到第60帧` 或 `第10帧到第60帧`。 |
...
截取后至少必须保留 2 帧；保留范围不能超过原动图总帧数。对超长动图，插件会**先**按本命令确定保留范围，再按 `max_frames` 在保留范围内均匀采样，因此不会从已经删掉的开头或结尾取帧。
...
回复或附上一张动图即可读取本地元数据。输出包括格式、容器版本、尺寸、文件大小、颜色模式、透明度、动画状态、**原始帧数**、循环次数、帧时长范围与平均值，以及容器携带的元数据键；帧数不受 `max_frames` 截断。帧数不超过 5000 时，还会给出总时长和平均 FPS；更长的动图会明确标注帧时长统计为抽样结果。背景索引、透明色索引、DPI 和注释存在时也会一并显示。

静态图片同样可以查询，会显示“动画：否”和帧数 1。
...
不带参数，返回一张高分辨率、大字号 PNG 指令速查图，覆盖全部 26 个注册命令、正式别名、素材类型和关键参数。图中会特别标明动图裁剪的位置参数、`gif截取` 的删帧/保留范围语义、时间和边距写法；完整的参数边界和配置说明仍以本 README 为准。

速查图作为插件内置 PNG 资源随发布包提供，不依赖 AstrBot 容器安装中文字体；采用更清晰的字重、较大字号与高分辨率画布，适合 QQ 中直接查看或点开阅读。
